...
static HashMap<String,String> huffMap;
static String encoder;
 ...
 public BinaryNode( HuffNode theElement, BinaryNode lt, BinaryNode rt )
 {
     element = theElement;
     left    = lt;
     right   = rt;
 ...
 public HashMap<String,String> getHuffman(int size)
 // Return a map of characters and their Huffman encoding
 {
	huffMap = new HashMap<String,String>(size);
	encoder = "";
	getHuffman();
	return huffMap;
 }
 ...
 private void getHuffman(){
	 //preorder traversal to return hashmap of huffman encoding
	 if (element.isValid == 1){     // Node
		 huffMap.put(element.theChar, encoder); //add to hashmap
     }
     if( left != null ){
     	 encoder += '0';
         left.getHuffman();			// Left
         encoder = encoder.substring(0, encoder.length()-1);
	 }
     if( right != null ){
    	 encoder += '1';
         right.getHuffman( );          // Right
         encoder = encoder.substring(0, encoder.length()-1);
     }
 }
 
 public HashMap<String,String> getTree(int size)
 // Return a map of characters and their Huffman encoding
 {
	huffMap = new HashMap<String,String>(size);
	encoder = "";
 ...
 public void setElement( HuffNode x )
 {
     element = x;
 }
